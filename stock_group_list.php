...
<div class="row">     
	<div class="col-md-2">       
	<h4 class="fw-bold mb-4" style="padding-top:2px;">Stock Group Master</h4>
	</div>
	<div class="col-md-2">
...
<div class="card">
  <div class="card-datatable table-responsive pt-0">
	...
	<tbody>
	   <?php
			$i=1;
			$data=$utilObj->getMultipleRow("stock_group","1");
			foreach($data as $info){
				
					$href= 'stock_group_list.php?id='.$info['id'].'&PTask=view';
		...
		<td>
            <!--div class="dropdown"-->
			  ...
			  <?php if((CheckEditMenu())==1) {  ?>
               <a class="dropdown-item" href="stock_group_list.php?id=<?php echo $info['id'];?>&PTask=update"><i class="bx bx-edit-alt me-1"></i> Edit</a>
			   <?php } ?>
			   <?php if((CheckDeleteMenu())==1){ ?>
                <a class="dropdown-item" href="stock_group_list.php?id=<?php echo $info['id'];?>&PTask=delete"><i class="bx bx-trash me-1"></i> Delete</a>
				<?php } ?>
              </div>
            <!--/div-->
...
<?php 
include("form/stock_group_form.php");
?>
...
if($_REQUEST['PTask']=='delete'){?>	
 window.onload=function(){
	var r=confirm("Are you sure to delete?");
		...
		{
			window.location="stock_group_list.php";
		}
 ...
 function CheckDelete()
{
	...
	else
	{
			...
			window.location="stock_group_list.php?PTask=delete&id="+val; 
...
function savedata(){
	...
	jQuery.ajax({url:'handler/stock_group_form.php', type:'POST',
				data: { PTask:PTask,id:id,name:name,under_group:under_group,negative_stk_block:negative_stk_block,table:table,LastEdited:LastEdited},
