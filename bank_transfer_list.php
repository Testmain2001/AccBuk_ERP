...
<div class="container-xxl flex-grow-1 container-p-y ">
	...
	<div class="row">     
		<div class="col-md-2">       
		<h4 class="fw-bold mb-4" style="padding-top:2px;">Contra</h4>
		</div>
		<div class="col-md-2">
...
<div class="card">
  <div class="card-datatable table-responsive pt-0">
	...
	<tbody>
	   <?php
			$i=1;
			$data1=$utilObj->getMultipleRow("bank_transfer"," 1 group by parent_id ");
			
			foreach($data1 as $info1){
                $j=0;
                
                $href= 'bank_transfer_list.php?parent_id='.$info1['parent_id'].'&PTask=view';
                $data=$utilObj->getMultipleRow("bank_transfer","parent_id='".$info1['parent_id']."'");
		...
		<td class='controls <?php echo $hidetd ;?>' rowspan='<?php echo $rowspan ;?>' >
            <!--div class="dropdown"-->
			  ...
			  <?php if((CheckEditMenu())==1) {  ?>
               <a class="dropdown-item" href="bank_transfer_list.php?parent_id=<?php echo $info['parent_id'];?>&PTask=update"><i class="bx bx-edit-alt me-1"></i> Edit</a>
			   <?php } ?>
			   <?php if((CheckDeleteMenu())==1){ ?>
                <a class="dropdown-item" href="bank_transfer_list.php?parent_id=<?php echo $info['parent_id'];?>&PTask=delete"><i class="bx bx-trash me-1"></i> Delete</a>
				<?php } ?>
              </div>
            <!--/div-->
...
<?php 
include("form/bank_transfer_form.php");
?>
...
if($_REQUEST['PTask']=='delete'){?>	
 window.onload=function(){
	var r=confirm("Are you sure to delete?");
		...
		{
			window.location="bank_transfer_list.php";
		}
...
function CheckDelete()
{
	...
	else
	{
			...
			window.location="bank_transfer_list.php?PTask=delete&parent_id="+val; 
