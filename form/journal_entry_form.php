<?php
	...
	if( $_REQUEST['PTask']=='update' || $_REQUEST['PTask']=='view' )
	{
		...
		$jornal_entry=mysqli_query($GLOBALS['con'],"Select * from journal_entry WHERE parent_id ='".$parent_id."'");
		...
		// $getinvno=mysqli_query($GLOBALS['con'],"Select max(	recordnumber) as pono from journal_entry WHERE ClientID ='".$_SESSION['Client_Id']."'");
    ...
    <div class="modal-content p-3 p-md-5">
		<div class="modal-body ">
			...
			<div class="text-center mb-4">
			  <h3 class="role-title">Journal Entry</h3>
			  
			</div>
			<!-- Add role form -->
			
				<form id="" data-parsley-validate class="row g-3" action="../journal_entry_list.php"  method="post" data-rel="myForm">
					...
					<input type="hidden"  name="table" id="table" value="<?php echo "journal_entry";?>"/>
						...
						<div class="col-md-4">
							...
							<select id="voucher_type" name="voucher_type"    <?php  echo $disabled ;?> class="required form-select select2" data-allow-clear="true" onchange = "get_journal_code();">
							<option value="">Select</option>
								<?php	
									$data=$utilObj->getMultipleRow("voucher_type","parent_voucher=3 group by id"); 
						...
						<div class=" text-nowrap" style=""><!--table-responsive-->
							...
							?>
								...
								<td style="width:10%">
									...
									$Account=$utilObj->getMultipleRow("account_ledger","1 group by name");   
...
function get_journal_code() {

	var voucher_type = $("#voucher_type").val();

	jQuery.ajax({url:'get_ajax_values.php', type:'POST',
		data: { Type:'get_journal_code',voucher_type:voucher_type},
