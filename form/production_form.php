...
// $getbatchno=mysqli_query($GLOBALS['con'],"Select MAX(batch_no) AS pono from production");
    ...
    <div class="modal-content p-3 p-md-5">
      <div class="modal-body ">
        ...
        <div class="text-center mb-4">
          <h3 class="role-title">Production</h3>
        ...
        <!-- Add role form -->
         ...
         <form id="" data-parsley-validate class="row g-3" action="../bill_of_material_list.php"  method="post" data-rel="myForm">
			...
			<input type="hidden"  name="table"      id="table"      value="<?php echo "production"; ?>"/>
					...
					<select id="location" name="location"  onchange="get_billofmaterial_rowtable();" <?php echo $disabled;?> class="select2 form-select required" data-allow-clear="true"  style="width:100%;">	
							<?php 
					...
					<select id="product" name="product" <?php echo $disabled;?> class="select2 form-select required" data-allow-clear="true" onchange="get_unit_forproduction();" style="width:100%;">	
							<?php 
								echo '<option value="">Select</option>';
								$record=$utilObj->getMultipleRow("stock_ledger","bill_of_material=1 AND id in(select product from bill_of_material where BOM_type='production') ");
								foreach($record as $e_rec)
								{
					...
					<input type="text" id="qty" class="required form-control"  onchange="get_billofmaterial_rowtable();" onblur="get_billofmaterial_rowtable();" onkeyup="get_billofmaterial_rowtable();" <?php echo $readonly;?> placeholder="Enter Quantity" name="qty" value="<?php echo $rows['qty'] ;?>"/>
					</div>
...
function get_billofmaterial_rowtable()
{
	//alert('hii');
			...
			data: { Type:'get_billofmaterial_rowtable',product:product,unit:unit,qty:qty,id:id,PTask:PTask,location:location},
