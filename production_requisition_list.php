...
<div class="container-xxl flex-grow-1 container-p-y ">
	<div class="row">     
		<div class="col-md-3">       
			<h4 class="fw-bold mb-4" style="padding-top:2px;">Stock Requisition</h4>
		</div>
		<div class="col-md-2">
			...
			<table class="datatables-basic table border-top" id="datatable-buttons" role="grid">
				...
				?>
					<tr>
						...
						<?php $username=$utilObj->getSingleRow("employee","id='".$info1['user']."'"); ?>
						...
						<?php if($d1==0) { ?>
							...
							<div class="dropdown-menu">
								<?php if((CheckEditMenu())==1) {  ?>
									<a class="dropdown-item" href="production_requisition_list.php?id=<?php echo $info1['id'];?>&PTask=update"><i class="bx bx-edit-alt me-1"></i> Edit</a>
								<?php } ?>
								<?php if((CheckDeleteMenu())==1){ ?>
									<a class="dropdown-item" href="production_requisition_list.php?id=<?php echo $info1['id'];?>&PTask=delete"><i class="bx bx-trash me-1"></i> Delete</a>
								<?php } ?>
							</div>
						<?php } ?>
						<?php
	...
	<?php 
		include("form/production_requisition_form.php");
	?>
...
<script>
	...
	if($_REQUEST['PTask']=='delete'){?>	
		window.onload=function(){
			var r=confirm("Are you sure to delete?");
				...
				{
					window.location="production_requisition_list.php";
				}
	...
	function CheckDelete()
	{
		...
		else
		{
				...
				window.location="production_requisition_list.php?PTask=delete&id="+val; 
	...
	function savedata() {
		...
		jQuery.ajax({url:'handler/production_requisition_form.php', type:'POST',
			data: { PTask:PTask,id:id,cnt:cnt,date:date,table:table,LastEdited:LastEdited,requisition_by:requisition_by,unit_array:unit_array,product_array:product_array,qty_array:qty_array,location:location,dep_type:dep_type,record_no:record_no,from_location:from_location },
