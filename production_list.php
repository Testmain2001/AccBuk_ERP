...
<div class="container-xxl flex-grow-1 container-p-y ">
	...
	<!-- <div class="row">     
		<div class="col-md-3">       
		<h4 class="fw-bold mb-4" style="padding-top:2px;">Production Details</h4>
		</div>
		<div class="col-md-2">
	...
	<div class="row">     
		<div class="col-md-2">       
		<h4 class="fw-bold mb-4" style="padding-top:2px;">Production</h4>
		</div>
		<div class="col-md-2">
		<?php if((CheckCreateMenu())==1){ ?> <button type="button" class="add_new btn btn-primary btn-sm" onclick="hideshow();" id="add_new" name="add_new">
			<i class="fas fa-plus-circle fa-lg"></i>
		</button>
		<?php } ?>
		<?php if((CheckDeleteMenu())==1){ ?> <button type="button" class="btn btn-danger btn-sm" onclick="CheckDelete();" id="delete" name="delete">
...
<!-- Invoice List Table -->
		...
		<div class="card">
			<div class="card-datatable table-responsive pt-0">
				...
				<table class="datatables-basic table border-top" id="datatable-buttons" role="grid">
					...
					<?php
						$i=1;
						$data=$utilObj->getMultipleRow("production","1");
						...
						<tr>
							...
							<td> <?php echo $info['production_code']; ?> </td>
							...
							if($d1==0){?>
								...
								<a class="dropdown-item" href="production_list.php?id=<?php echo $info['id'];?>&PTask=update"><i class="bx bx-edit-alt me-1"></i> Edit</a>
								<?php } ?>
								<?php if((CheckDeleteMenu())==1){ ?>
									<a class="dropdown-item" href="production_list.php?id=<?php echo $info['id'];?>&PTask=delete"><i class="bx bx-trash me-1"></i> Delete</a>
									<?php } ?>
								</div>
								<!--/div-->
							<?php }?>
	...
	<?php
		if($_REQUEST['PTask']=='update' || $_REQUEST['PTask']=='view'){
			$id = $_REQUEST['id'];
			$rows=$utilObj->getSingleRow("production","id ='".$id."'");
			$batch=$utilObj->getSingleRow("purchase_batch","parent_id ='".$id."'");
			// print_r($rows);
			$batch_no=$rows['batch_no'];
			$date=date('d-m-Y',strtotime($rows['date']));
			$mfg_date=date('d-m-Y',strtotime($batch['mfg_date']));
			$exp_date=date('d-m-Y',strtotime($batch['exp_date']));
		...
		<div class="row form-validate">
			<div class="col-12">
				<div class="card ">
					...
					<form id="demo-form2" data-parsley-validate class="row g-3" action="production_list.php"  method="post" data-rel="myForm">
							...
							<input type="hidden"  name="table"      id="table"      value="<?php echo "production"; ?>"/>
							...
							<div class="col-md-4">
								...
								<select id="voucher_type" name="voucher_type"    <?php  echo $disabled ;?> class="required form-select select2" data-allow-clear="true" onchange = "get_production_code();">
								<option value="">Select</option>
									<?php	
										$data=$utilObj->getMultipleRow("voucher_type","parent_voucher=11 group by id"); 
										foreach($data as $info){
							...
							<div class="col-md-4">
								<label class="form-label">Production  Date <span class="required required_lbl" style="color:red;">*</span></label>
							...
							<div class="col-md-4">
								...
								<select id="product" name="product" <?php echo $disabled;?> class="select2 form-select required" data-allow-clear="true" onchange="get_unit_forproduction();get_bom();" style="width:100%;">	
									<?php 
										echo '<option value="">Select</option>';
							...
							<div class="col-md-4">
								...
								<div id='unitdiv'>
									<input type="text" style="width:100%;"  class=" form-control  smallinput " onchange="get_billofmaterial_rowtable();" readonly id="unit" <?php echo $readonly;?> name="unit" value="<?php echo $rows['unit'];?>"/>
								</div>
							</div>
											
							<div class="col-md-4">
								<label class="form-label">Quantity <span class="required required_lbl" style="color:red;">*</span></label>
								<input type="text" id="qty" class="tdalign required form-control"  onchange="get_billofmaterial_rowtable();" <?php echo $readonly;?> placeholder="Enter Quantity" name="qty" value="<?php echo $rows['qty'] ;?>"/>
							</div>

							<div class="col-md-4">
								<label class="form-label">Batch Name<span class="required required_lbl" style="color:red;">*</span></label>
								<input type="text" id="pro_batch_name" name="pro_batch_name" class="required form-control" placeholder="Enter Batch Name" value="<?php echo $batch['batchname'] ;?>"/>
							</div>
							...
							<div class="col-md-4">
								<label class="form-label">MFG Date<span class="required required_lbl" style="color:red;">*</span></label>
								<input type="text" class="form-control flatpickr" id="mfg_date" name="mfg_date" required value="<?php echo $date;?>" <?php echo $disabled;?>/>
							</div>

							<div class="col-md-4">
								<label class="form-label">EXP Date<span class="required required_lbl" style="color:red;">*</span></label>
								<input type="text" class="form-control flatpickr" id="exp_date" name="exp_date" required value="<?php echo $date;?>" <?php echo $disabled;?>/>
							</div>
								...
								?>
									<?php if((CheckEditMenu())==1) { ?>
										<button type="button" class="add_new btn btn-warning" onclick="hideshow();" id="add_new" name="add_new">
											<a href="production_list.php?id=<?php echo $_REQUEST['id']; ?>&PTask=update">Edit</a>
										</button>
									<?php } ?>
								<?php } ?>
	...
	<?php // include("form/production_form.php"); ?>
</div>
...
function get_bom() {
	...
	jQuery.ajax({url:'get_ajax_values.php', type:'POST',
		data: { Type:'get_bom',product:product },
...
function get_production_code() {
	...
	jQuery.ajax({url:'get_ajax_values.php', type:'POST',
		data: { Type:'get_production_code',voucher_type:voucher_type},
		success:function(data)
		{	
			// alert(data);
			$("#batch_no").val(data);	
...
function get_unit_forproduction()
{	

	var product = $("#product").val();
	// alert(product);
	jQuery.ajax({url:'get_ajax_values.php', type:'POST',
		data: { Type:'get_unit_forproduction',product:product},
		success:function(data)
		{	
			// alert(data);
			$("#unitdiv").html(data);	
...
function get_billofmaterial_rowtable()
{
	var product=$("#product").val();
	var bom=$("#bom").val();
	var unit=$("#unit").val();
	...
	jQuery.ajax({url:'get_ajax_values.php',
		type:'POST',
		data: { Type:'get_billofmaterial_rowtable',product:product,unit:unit,qty:qty,id:id,PTask:PTask,location:location,ad:ad,bom:bom },
...
function CheckDelete()
{
	...
	else
	{
		...
		window.location="production_list.php?PTask=delete&id="+val; 
...
function savedata()
{
	...
	var unit = $("#unit").val();
	var qty = $("#qty").val();
	var pro_batch_name = $("#pro_batch_name").val();
