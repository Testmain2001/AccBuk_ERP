...
<table class="datatables-basic table border-top" id="datatable-buttons" role="grid">
				...
				{
				$data=$utilObj->getMultipleRow("sale_receipt","ClientID='".$_SESSION['Client_Id']."' AND customer ='".$id."'  order by receiptdate DESC,id DESC");
				}
				foreach($data as $info)	
				{
					$i++;
					$total=$info['amt_pay']; 
					$date=date_create($info['receiptdate']);
					...
					echo "<tr class='even'>  ";
					echo "<td style='width:3%;' class='controls'><input type='checkbox' class='es' $dasable   name='check_list' value='".$info['id']."' /> </td> ";
				
					echo "<td style='width:10%;' class='controls'><a href='sale_receipt_list.php?id=".$info['id']."&PTask=view' >".$Date."</a></td>";
					...
					if(empty($dasable))
					{
					 echo "<td style='width:5%;'><a href='sale_receipt_list.php?id=".$info['id']."&PTask=update' ata-content='Edit' title='Edit' class='popovers' data-placement='top' data-trigger='hover'  ><i class='fa fa-edit' ></i></a>";
...
if($_REQUEST['PTask']=='delete'){?>	
 window.onload=function(){
	var r=confirm("Are you sure to delete?");
		...
		{
			window.location="sale_history.php";
		}
...
function CheckDelete()
{
	...
	else
	{
			...
			window.location="sale_history.php?PTask=delete&id="+val; 
