...
<div class="container-xxl flex-grow-1 container-p-y ">
	...
	<div class="row">     
		<div class="col-md-3">       
			<h4 class="fw-bold mb-4" style="padding-top:2px;">Bank Receipt</h4>
		</div>
		<div class="col-md-2">
			...
			<table class="datatables-basic table border-top" id="datatable-buttons" role="grid">
				...
				<?php
					...
					if($_REQUEST['Task']=='')
					{
						...
						$data = mysqli_query($GLOBALS['con'],"SELECT account_ledger.id as id FROM account_ledger  where account_ledger.group_name='14' AND id in( select customer from sale_receipt where 1) group by account_ledger.id ");
					}
					...
					while($info=mysqli_fetch_array($data))
					{
						...
						$getsum=$utilObj->getSum("sale_receipt","customer='".$vendr['id']."'","amt_pay"); 
						...
						echo "<td >".$status. "</td>"; 									
						echo "<td><a ata-content='Make Payment' title='Make Payment' class='popovers' data-placement='top'  data-trigger='hover' $dasable  href='sale_receipt_list.php?customer=".$vendr['id']."&PTask=makepayment' >
								<i class='fa fa-money' style='color: crimson;'></i></a>";
								
						echo "<a ata-content='History' title='History' class='popovers' data-placement='top' data-trigger='hover' $dasable href='sale_history.php?customer=".$vendr['id']."&Task=history' >
	...
	<?php 
		include("form/sale_receipt_form.php");
	?>
...
<script>
	...
	{	 
		window.location="sale_receipt_list.php";
	} 
	...
	function savedata()
	{
		...
		jQuery.ajax({url:'handler/sale_receipt_form.php', type:'POST',
			data: { PTask:PTask,table:table,LastEdited:LastEdited,id:id,cnt:cnt,recordnumber:recordnumber,voucher_type:voucher_type,date:date,location:location,customer:customer,type:type,totalvalue:totalvalue,mode:mode,bankid:bankid,balance:balance,cheque_no:cheque_no,amt_pay:amt_pay,narration:narration,type_array:type_array,billno_array:billno_array,invodate_array:invodate_array,totalinvo_array:totalinvo_array,pendingamt_array:pendingamt_array,payamt_array:payamt_array,totalvalue:totalvalue },
