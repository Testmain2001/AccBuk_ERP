...
<div class="row">     
	<div class="col-md-3">       
	<h4 class="fw-bold mb-4" style="padding-top:2px;">Sales Order</h4>
	</div>
	<div class="col-md-2">
...
<div class="card">
  <div class="card-datatable table-responsive pt-0" style="overflow-x: auto;">
	...
	<tbody>
	   <?php
			$i=0;
			$data=$utilObj->getMultipleRow("sale_order","1");
			foreach($data as $info)
			{
					...
					foreach($data1 as $info1)
					{
						...
						<tr>
							...
							<td class="<?php echo $hidetd; ?>" rowspan="<?php echo $rowspan; ?>"><a href="<?php echo $href; ?>"><?php echo $info['order_no']; ?></a> </td>
							...
							?>
								...
								<?php if((CheckEditMenu())==1) {  ?>
									<a class="dropdown-item" href="sale_order_list.php?id=<?php echo $info['id'];?>&PTask=update"><i class="bx bx-edit-alt me-1"></i> Edit</a>
								<?php } ?>
								<?php if((CheckDeleteMenu())==1){ ?>
									<a class="dropdown-item" href="sale_order_list.php?id=<?php echo $info['id'];?>&PTask=delete"><i class="bx bx-trash me-1"></i> Delete</a>
								<?php } ?>
							  	</div>
							<!--/div-->
							<?php // } ?>
...
<?php 
include("form/sale_order_form.php");
?>
...
if($_REQUEST['PTask']=='delete'){?>	
 window.onload=function(){
	...
	{
		window.location="sale_order_list.php";
	}
...
function CheckDelete()
{
	...
	else
	{
			...
			window.location="sale_order_list.php?PTask=delete&id="+val; 
...
function savedata()
{
	...
	jQuery.ajax({url:'handler/sale_order_form.php', type:'POST',
		data: { PTask:PTask,table:table,LastEdited:LastEdited,id:id,cnt:cnt,order_no:order_no,date:date,customer:customer,location:location,unit_array:unit_array,product_array:product_array,cgst_array:cgst_array,sgst_array:sgst_array,igst_array:igst_array,qty_array:qty_array,rate_array:rate_array,state_name:state_name,state_code:state_code,pos_state:pos_state,ledger_array:ledger_array,bill_to:bill_to,ship_to:ship_to,taxable_array:taxable_array,grandtotal:grandtotal,cntd:cntd,serviceledger_array:serviceledger_array,servicecgst_array:servicecgst_array,servicesgst_array:servicesgst_array,serviceigst_array:serviceigst_array,serviceamt_array:serviceamt_array,totdiscount:totdiscount,totaltaxable:totaltaxable,cgstledger:cgstledger,cgstamt:cgstamt,sgstledger:sgstledger,sgstamt:sgstamt,igstledger:igstledger,igstamt:igstamt,subtotgst:subtotgst,totserviceamt:totserviceamt },
