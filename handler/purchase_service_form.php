...
if(isset($_REQUEST['PTask']))
	{
        switch($_REQUEST['PTask'])	
		{
			case "Add":
				...
				// print_r($arrValue);
				$insertedId=$utilObj->insertRecord('purchase_invoice_service',$arrValue);
				...
				for($i=0;$i<$cnt1;$i++)
				{
					...
					// print_r($arrValue2);
					$insertedId=$utilObj->insertRecord('purchase_invoice_service_details', $arrValue2);
				...
				for($i=0;$i<$cntad;$i++) {
					...
					$form_type = 'purchase_invoice_service';
			...
			case "update":
				...
				$mate1=$utilObj->getSingleRow("purchase_invoice_service","id='".$_REQUEST['id']."'");
				$mate3=$utilObj->getSingleRow("voucher_type","id='".$_REQUEST['voucher_type']."'");
				...
				$Updaterec=$utilObj->updateRecord('purchase_invoice_service', $strWhere, $arrValue);
				
				$strWhere="parent_id='".$_REQUEST['id']."' ";
				$Deleterec=$utilObj->deleteRecord('purchase_invoice_service_details', $strWhere);
			...
			foreach($pids as $pid)
			{
				...
				$strWhere="id='".$pid."' ";
				$Deleterec=$utilObj->deleteRecord('purchase_invoice_service', $strWhere);
