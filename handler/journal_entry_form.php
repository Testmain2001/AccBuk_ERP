        ...
        switch($_REQUEST['PTask'])	
		{
			case "Add":
				...
				for($i=0;$i<$cnt1;$i++)
				{
					...
					$arrValue=array('id'=>$id1,'parent_id'=>$id,'user'=>$_SESSION['Ck_User_id'],'Created'=>date("Y-m-d H:i:s"),
					'LastEdited'=>date('Y-m-d H:i:s'),'ClientID'=>$_SESSION['Client_Id'],'date'=>date('Y-m-d',strtotime($_REQUEST['date'])),'recordnumber'=>$pino,'voucher_code'=>$pur_ino_code,'voucher_type'=>$_REQUEST['voucher_type'],'total_of_debitamt'=>$_REQUEST['total_of_debitamt'],'total_of_creditamt'=>$_REQUEST['total_of_creditamt'],'record'=>$_REQUEST['record_array'][$i],'account'=>$_REQUEST['account_array'][$i],'debit_amount'=>$_REQUEST['debit_amount_array'][$i],'credit_amount'=>$_REQUEST['credit_amount_array'][$i]);
					//print_r($arrValue2);
					 $insertedId=$utilObj->insertRecord('journal_entry', $arrValue);
			...
			case "update":
				...
				$mate1=$utilObj->getSingleRow("journal_entry","parent_id='".$_REQUEST['parent_id']."'");
				$mate3=$utilObj->getSingleRow("voucher_type","id='".$_REQUEST['voucher_type']."'");
				...
				$Deleterec=$utilObj->deleteRecord('journal_entry', $strWhere);
				...
				for($i=0;$i<$cnt1;$i++)
				{
					...
					$arrValue=array('id'=>$id1,'parent_id'=>$id,'Created'=>date("Y-m-d H:i:s"),'LastEdited'=>date('Y-m-d H:i:s'),'ClientID'=>$_SESSION['Client_Id'],'date'=>date('Y-m-d',strtotime($_REQUEST['date'])),'recordnumber'=>$pino,'voucher_code'=>$pur_ino_code,'voucher_type'=>$_REQUEST['voucher_type'],'total_of_debitamt'=>$_REQUEST['total_of_debitamt'],'total_of_creditamt'=>$_REQUEST['total_of_creditamt'],'record'=>$_REQUEST['record_array'][$i],'account'=>$_REQUEST['account_array'][$i],'debit_amount'=>$_REQUEST['debit_amount_array'][$i],'credit_amount'=>$_REQUEST['credit_amount_array'][$i],);
					print_r($arrValue2);
					 $insertedId=$utilObj->insertRecord('journal_entry', $arrValue);
