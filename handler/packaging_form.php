...
if(isset($_REQUEST['PTask']))
	{
        switch($_REQUEST['PTask'])	
		{
			case "Add":
				...
				if ($mate3['numbering_digit'] == 'Prefix') {
					...
					if ($mate1['voucher_type'] != '') {
						...
						$pack_code = $prefix_label."/".($formattedPono)."/".$year_code;
						$pcno = $formattedPono;
					} 
						...
						$pack_code = $prefix_label."/".($result['pono'])."/".$year_code;
						$pcno = $result['pono'];
					}
				}
				else {
					...
					if ($mate1['voucher_type'] != '') {
						...
						$pack_code = $prefix_label."/".$year_code."/".($formattedPono);
						$pcno = $formattedPono;
					} 
						...
						$pack_code = $prefix_label."/".$year_code."/".($result['pono']);
						$pcno = $result['pono'];
					}
				}
				...
				// $id=uniqid();
				$arrValue=array('id'=>$ad,'user'=>$_SESSION['Ck_User_id'],'ClientID'=>$_SESSION['Client_Id'],'Created'=>date("Y-m-d H:i:s"),'LastEdited'=>date('Y-m-d H:i:s'),'batch_no'=>$pcno,'pack_code'=>$pack_code,'location'=>$_REQUEST['location'],'voucher_type'=>$_REQUEST['voucher_type'],'product'=>$_REQUEST['product'],'unit'=>$_REQUEST['unit'],'qty'=>$_REQUEST['qty'],'date'=>date('Y-m-d',strtotime($_REQUEST['date'])),'bom'=>$_REQUEST['bom'],'rate'=>$_REQUEST['pro_batch_rate'],'total_req'=>$_REQUEST['total_req'],'grand_total'=>$_REQUEST['grand_total'] );
				// echo "hiii";
				// print_r($arrValue);
				$insertedId=$utilObj->insertRecord('packaging',$arrValue);

				// --------------------------------- Packaging Batch ---------------------------------
				$type="packaging_in";

				$arrValue1=array('id'=>uniqid(),'parent_id'=>$ad,'ClientID'=>$_SESSION['Client_Id'],'location'=>$_REQUEST['location'],'date'=>date('Y-m-d',strtotime($_REQUEST['date'])),'CreatedAt'=>date("Y-m-d H:i:s"),'LastEdited'=>date('Y-m-d H:i:s'),'type'=>$type,'batchname'=>$_REQUEST['pack_batch_name'],'batqty'=>$_REQUEST['qty'],'product'=>$_REQUEST['product'],'bat_rate'=>$_REQUEST['pro_batch_rate'],'bom_id'=>$_REQUEST['bom'] );
				...
				for($i=0;$i<$cnt1;$i++)
				{
					...
					// print_r($arrValue2);
					$insertedId=$utilObj->insertRecord('packaging_details', $arrValue2);
					// }
		        }
			...
			case "update":
				...
				$mate1=$utilObj->getSingleRow("packaging","id='".$_REQUEST['id']."'");
				$mate3=$utilObj->getSingleRow("voucher_type","id='".$_REQUEST['voucher_type']."'");
				...
				$arrValue=array('LastEdited'=>date('Y-m-d H:i:s'),'batch_no'=>$pcno,'pack_code'=>$pack_code,'location'=>$_REQUEST['location'],'voucher_type'=>$_REQUEST['voucher_type'],'product'=>$_REQUEST['product'],'unit'=>$_REQUEST['unit'],'qty'=>$_REQUEST['qty'],'date'=>date('Y-m-d',strtotime($_REQUEST['date'])),'bom'=>$_REQUEST['bom'],'rate'=>$_REQUEST['pro_batch_rate'],'total_req'=>$_REQUEST['total_req'],'grand_total'=>$_REQUEST['grand_total'] );
				//print_r($arrValue);
				$strWhere="id='".$_REQUEST['id']."'  ";
				$Updaterec=$utilObj->updateRecord('packaging', $strWhere, $arrValue);
				
				$strWhere="parent_id='".$_REQUEST['id']."' ";
				$Deleterec=$utilObj->deleteRecord('packaging_details', $strWhere);
			...
			case"delete":
				...
				{
					$strWhere="id='".$pid."' ";
					$Deleterec=$utilObj->deleteRecord('packaging', $strWhere);
