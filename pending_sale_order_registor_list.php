...
<div class="row">     
	<div class="col-md-3">       
	<h4 class="fw-bold mb-4" style="padding-top:2px;">Pending Sale Order Registor </h4>
	</div>
	...
	<table class="datatables-basic table border-top" id="datatable-buttons" role="grid">
		<thead>
			<tr>
				...
				<th width='10%'>Unit</th>
				<th width='10%'>Pending Sale Order Quantity</th>
			</tr>
		</thead>
		...
		<tbody>
		   <?php
				...
				$data=$utilObj->getMultipleRow("sale_order","$cnd");//AND id NOT IN(select saleorder_no from delivery_challan where 1)
				foreach($data as $info) {
					$i++;$j=0;$k=0;
					...
					$data1=$utilObj->getMultipleRow("sale_order_details","parent_id='".$info['id']."'");
				  	foreach($data1 as $info1){
						...
						$delivery_challan_details=$utilObj->getSingleRow("delivery_challan_details","product='".$info1['product']."' AND unit='".$info1['unit']."' AND parent_id in(select  id  from delivery_challan where  saleorder_no='".$info['id']."' AND  customer='".$info['customer']."'   )  ");

						if($delivery_challan_details) {

							$k++;
							// var_dump($delivery_challan_details);
							
							$pending_qty=$info1['qty']-$delivery_challan_details['qty'];
							/* if($j==1){
								$rowspan=Count($info1);
								$hidetd="";
				...
				?>
					<tr>
						...
						<td class="<?php echo $hidetd; ?>" rowspan="<?php echo $rowspan; ?>"><a href="<?php echo $href; ?>"><?php echo $info['order_no']; ?></td>
...
function Search(){
	...
	window.location="pending_sale_order_registor_list.php?FromDate="+fromdate+"&ToDate="+todate+"&customer="+customer+"&Task=filter";
