...
<table class="datatables-basic table border-top" id="datatable-buttons" role="grid">
				...
				{
				$data=$utilObj->getMultipleRow("purchase_payment","ClientID='".$_SESSION['Client_Id']."' AND supplier ='".$id."'  order by paymentdate DESC,id DESC");
				}
				foreach($data as $info)	
				{
					...
					echo "<td style='width:10%;' class='controls'><a href='purchase_payment_list.php?id=".$info['id']."&PTask=view' >".$Date."</a></td>";
					...
					if(empty($dasable))
					{
					 echo "<td style='width:5%;'><a href='purchase_payment_list.php?id=".$info['id']."&PTask=update' ata-content='Edit' title='Edit' class='popovers' data-placement='top' data-trigger='hover'  ><i class='fa fa-edit' ></i></a>";
...
function CheckDelete()
{
	...
	else
	{
			...
			window.location="purchase_history.php?PTask=delete&id="+val; 
