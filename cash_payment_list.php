...
<div class="row">     
	<div class="col-md-3">       
	<h4 class="fw-bold mb-4" style="padding-top:2px;">Cash Payment</h4>
	</div>
	<div class="col-md-2">
	<button class=" btn btn-primary mr-2  btn-sm" data-bs-target="#makepaymentModal" data-bs-toggle="modal" data-bs-dismiss="modal" id="makepayment">Cash Payment</button>
	</div>
</div>
<!-- Invoice List Table -->
  ...
  <div class="card-datatable table-responsive pt-0">
	...
	$i=0;
								if($_REQUEST['Task']=='')
								{
									...
									$data = mysqli_query($GLOBALS['con'],"SELECT account_ledger.id as id FROM account_ledger  where account_ledger.group_name='18' AND id in( select supplier from  cash_payment where 1)  group by account_ledger.id ");
								...
								while($info=mysqli_fetch_array($data))
								{
									...
									// $getdebit=$utilObj->getSum("cash_payment","PID='".$vendr['id']."' $cndloc ","debit_amt");
                                    $getsum=$utilObj->getSum("cash_payment","supplier='".$vendr['id']."'","amt_pay"); 
                                   	// $cheque=$utilObj->getSum("cash_payment","PID='".$vendr['id']."' $cndloc","cheque_amt");	
                                    $sum=($getsum);
                                    ...
                                    echo "<td >".$status. "</td>"; 									
									echo "<td><a ata-content='Make Payment' title='Make Payment' class='popovers' data-placement='top'  data-trigger='hover' $dasable  href='cash_payment_list.php?supplier=".$vendr['id']."&PTask=makepayment' >
			                	            <i class='fa fa-money' style='color: crimson;'></i></a>";
											
									echo "<a ata-content='History' title='History' class='popovers' data-placement='top' data-trigger='hover' $dasable href='cash_payment_history.php?supplier=".$vendr['id']."&Task=history' >
...
<?php 
	include("form/cash_payment_form.php");
?>
...
{	 
	window.location="cash_payment_list.php";
} 
...
function savedata()
{
	...
	var purchaseid_array=[];
		...
		for(var i=1;i<=cnt;i++)
		{
			...
			var purchaseid =$("#purchaseid"+i).val();
				
			bank_array.push(bank);
			bank1_array.push(bank1);
			purchaseid_array.push(purchaseid);
		} 
		
		jQuery.ajax({url:'handler/cash_payment_form.php', type:'POST',
			data: { PTask:PTask,table:table,LastEdited:LastEdited,id:id,cnt:cnt,recordnumber:recordnumber,voucher_type:voucher_type,date:date,supplier:supplier,type:type,totalvalue:totalvalue,mode:mode,bank_ledger:bank_ledger,balance:balance,cheque_no:cheque_no,amt_pay:amt_pay,narration:narration,bank_array:bank_array,bank1_array:bank1_array,purchaseid_array:purchaseid_array},
