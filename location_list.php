...
<div class="row">     
	<div class="col-md-2">       
	<h4 class="fw-bold mb-4" style="padding-top:2px;">Location Master</h4>
	</div>
	<div class="col-md-2">
...
<div class="card">
  <div class="card-datatable table-responsive pt-0">
	...
	<tbody>
	   <?php
			$i=1;
			$data=$utilObj->getMultipleRow("location","1");
			foreach($data as $info){
				
					$href= 'location_list.php?id='.$info['id'].'&PTask=view';
		...
		<td>
            <!--div class="dropdown"-->
			  ...
			  <?php if((CheckEditMenu())==1) {  ?>
               <a class="dropdown-item" href="location_list.php?id=<?php echo $info['id'];?>&PTask=update"><i class="bx bx-edit-alt me-1"></i> Edit</a>
			   <?php } ?>
			   <?php if((CheckDeleteMenu())==1){ ?>
                <a class="dropdown-item" href="location_list.php?id=<?php echo $info['id'];?>&PTask=delete"><i class="bx bx-trash me-1"></i> Delete</a>
				<?php } ?>
              </div>
            <!--/div-->
...
<?php 
include("form/location_form.php");
?>
 ...
 function CheckDelete()
{
	...
	else
	{
			...
			window.location="location_list.php?PTask=delete&id="+val; 
...
function savedata(){
	...
	var pos_check =$('input[name="pos_check"]:checked').val();
	var accour_led_id = $("#accour_led_id").val();

	var table = $("#table").val();
	var LastEdited = $("#LastEdited").val();
	
	jQuery.ajax({url:'handler/location_form.php', type:'POST',
				data: { PTask:PTask,id:id,name:name,under_group:under_group,negative_stk_block:negative_stk_block,pos_check:pos_check,accour_led_id:accour_led_id,table:table,LastEdited:LastEdited},
