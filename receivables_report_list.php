...
<div class="container-xxl flex-grow-1 container-p-y ">
	...
	<div class="row">     
		<div class="col-md-3">       
			<h4 class="fw-bold mb-4" style="padding-top:2px;">Receivable Registor Invoicewise</h4>
		</div>
	</div>
			...
			<table class="datatables-basic table border-top" id="datatable-buttons" role="grid">
				<thead>
					<tr>
						...
						<th width='10%'>Sale Invoice Amount</th>
						<th width='10%'>Paid Amount</th>
						<th width='10%'>Reveivable</th>
					</tr>
				</thead>
				...
				<?php
					...
					foreach($data as $info) {
						...
						$supplier=$utilObj->getSingleRow("account_ledger","id='".$info['customer']."'");
						...
						$pp_details=$utilObj->getSum("sale_receipt_details","parent_id in(select id from sale_receipt where customer='".$info['customer']."') AND saleid='".$info['id']."' ","amount");

						$pp_details_cash=$utilObj->getSum("cash_receipt_details","parent_id in(select id from cash_receipt where customer='".$info['customer']."') AND saleid='".$info['id']."' ","amount");
				...
				?>
						<tr>
							...
							<td><a href="<?php echo $href; ?>" target="_blank"><?php echo $info['saleino_code']; ?></td>
...
<script>
	...
	function Search(){
		...
		window.location="receivables_report_list.php?FromDate="+fromdate+"&ToDate="+todate+"&customer="+customer+"&method="+method+"&Task=filter";
	}
