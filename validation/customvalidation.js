...
function _accno(e) {
  ...
  if (!t.test(e.val())) {
    isError = true;
    promptText = promptText + "Please enter a valid Account No.<br />";
  }
}
function _gst(e) {
  var t = /^(^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$)+$/;
  if (!t.test(e.val())) {
    isError = true;
    promptText = promptText + "Please enter a GST name.<br />";
  }
}
function _phone(e) {
  if (e.val() != "") {
