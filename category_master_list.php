...
<div class="row">     
	<div class="col-md-2">       
	<h4 class="fw-bold mb-4" style="padding-top:2px;">Stock Category Master</h4>
	</div>
	<div class="col-md-2">
...
<div class="card">
  <div class="card-datatable table-responsive pt-0">
	...
	<tbody>
	   <?php
			$i=1;
			$data=$utilObj->getMultipleRow("stock_category_master","1");
			foreach($data as $info){
				
					$href= 'category_master_li.php?id='.$info['id'].'&PTask=view';
		...
		<td><?php echo $i; ?></td>
		<td> <a href="<?php echo $href; ?>"><?php echo $info['cat_name']; ?></a> </td>
		<td> <a href="<?php echo $href; ?>"><?php echo $info['under_group']; ?></a> </td>
		...
		<td>
            <!--div class="dropdown"-->
			  ...
			  <?php if((CheckEditMenu())==1) {  ?>
               <a class="dropdown-item" href="category_master_list.php?id=<?php echo $info['id'];?>&PTask=update"><i class="bx bx-edit-alt me-1"></i> Edit</a>
			   <?php } ?>
			   <?php if((CheckDeleteMenu())==1){ ?>
                <a class="dropdown-item" href="category_master_list.php?id=<?php echo $info['id'];?>&PTask=delete"><i class="bx bx-trash me-1"></i> Delete</a>
				<?php } ?>
              </div>
            <!--/div-->
...
<?php 
include("form/category_master_form.php");
?>
...
if($_REQUEST['PTask']=='delete'){?>	
 window.onload=function(){
	var r=confirm("Are you sure to delete?");
		...
		{
			window.location="category_master_list.php";
		}
 ...
 function CheckDelete()
{
	...
	else
	{
			...
			window.location="category_master_list.php?PTask=delete&id="+val; 
...
function savedata(){
	...
	jQuery.ajax({url:'handler/category_master_form.php', type:'POST',
		data: { PTask:PTask,id:id,name:name,under_group:under_group,table:table,LastEdited:LastEdited},
