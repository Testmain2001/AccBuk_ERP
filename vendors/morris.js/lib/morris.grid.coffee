    ...
    if typeof boundaryOption is 'string'
      if boundaryOption[0..3] is 'auto'
        if boundaryOption.length > 5
          suggestedValue = parseFloat(boundaryOption[5..], 10)
  ...
  if m
    ...
    if not q[6]
      ...
      new Date(
        ...
        parseFloat(q[3], 10),
        parseFloat(q[4], 10),
        parseFloat(q[5], 10)).getTime()
    else
      # timezone info supplied, use UTC
      offsetmins = 0
      if q[6] != 'Z'
        offsetmins = parseFloat(q[8], 10) * 60 + parseFloat(q[9], 10)
    ...
    if not r[8]
      ...
      offsetmins = 0
      if r[8] != 'Z'
        offsetmins = parseFloat(r[10], 10) * 60 + parseFloat(r[11], 10)
      ...
      Date.UTC(
        ...
        parseFloat(r[3], 10),
        parseFloat(r[4], 10),
        parseFloat(r[5], 10) + offsetmins,
        isecs,
        msecs)
  else
    new Date(parseFloat(date, 10), 0, 1).getTime()
