...
(function () {
      ...
      if (typeof boundaryOption === "string") {
        if (boundaryOption.slice(0, 4) === "auto") {
          if (boundaryOption.length > 5) {
            suggestedValue = parseFloat(boundaryOption.slice(5), 10);
          ...
          return parseFloat(boundaryOption, 10);
        }
      } else {
  ...
  Morris.parseDate = function (date) {
    ...
    if (m) {
      ...
      return ret.getTime() + parseFloat(p[2], 10) * 604800000;
    } else if (q) {
      if (!q[6]) {
        return new Date(
          parseFloat(q[1], 10),
      ...
      return new Date(parseFloat(date, 10), 0, 1).getTime();
    }
  };
