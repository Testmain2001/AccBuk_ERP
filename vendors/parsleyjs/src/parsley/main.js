...
if (parseFloat(vernums[0]) <= 1 && parseFloat(vernums[1]) < 8) {
  ...
  function (i, method) {
    ...
    window.ParsleyValidator[method] = function () {
      ParsleyUtils.warnOnce(
        `Accessing the method '${method}' through ParsleyValidator is deprecated. Simply call 'window.Parsley.${method}(...)'`
      );
...
$.each("addError updateError".split(" "), function (i, method) {
  window.ParsleyUI[method] = function (
    ...
    var updateClass = true !== doNotUpdateClass;
    ParsleyUtils.warnOnce(
      `Accessing ParsleyUI is deprecated. Call '${method}' on the instance directly. Please comment in issue 1073 as to your need to call this method.`
    );
