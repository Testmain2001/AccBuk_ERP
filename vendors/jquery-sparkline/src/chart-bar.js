...
$.fn.sparkline.bar = bar = createClass(
  ...
  barHighlightMixin,
  {
    ...
    init: function (el, values, options, width, height) {
      var barWidth = parseFloat(options.get("barWidth"), 10),
        barSpacing = parseFloat(options.get("barSpacing"), 10),
    ...
    renderRegion: function (valuenum, highlight) {
      ...
      for (i = 0; i < valcount; i++) {
        ...
        if (range > 0) {
          height =
            Math.floor(canvasHeightEf * (Math.abs(val - xaxisOffset) / range)) +
            1;
        } else {
          height = 1;
        }
