define(function (require) {
        ...
        doMergeOption: function (newOption, isInit) {
            ...
            this.resetVisual(function (mappingOption, state) {
                if (mode === "categories") {
                    ...
                    mappingOption.pieceList = zrUtil.map(
                        this._pieceList,
                        function (piece) {
    ...
    var resetMethods = {
        splitNumber: function () {
            ...
            splitNumber = Math.max(parseFloat(splitNumber, 10), 1);
            thisOption.splitNumber = splitNumber;

            var splitStep = (dataExtent[1] - dataExtent[0]) / splitNumber;
            // Precision auto-adaption
            while (
                +splitStep.toFixed(precision) !== splitStep &&
                precision < 5
            ) {
                precision++;
            }
            ...
            for (
                var i = 0, curr = dataExtent[0];
                i < splitNumber;
                i++, curr += splitStep
            ) {
                var max =
                    i === splitNumber - 1 ? dataExtent[1] : curr + splitStep;
