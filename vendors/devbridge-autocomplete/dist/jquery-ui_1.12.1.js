...
(function (factory) {
    ...
    var stepHooks =
      ...
      stringParsers = [
        ...
        },
        {
          ...
          parse: function (execResult) {
            return [
              parseFloat(execResult[1], 16),
              parseFloat(execResult[2], 16),
              parseFloat(execResult[3], 16),
            ];
          },
        },
        {
          ...
          parse: function (execResult) {
            return [
              parseFloat(execResult[1] + execResult[1], 16),
              parseFloat(execResult[2] + execResult[2], 16),
              parseFloat(execResult[3] + execResult[3], 16),
            ];
          },
        },
        {
  ...
  var effectsEffectFold = $.effects.define(
    ...
    function (options, done) {
      ...
      if (percent) {
        size = (parseFloat(percent[1], 10) / 100) * distance[hide ? 0 : 1];
      }
  ...
  var effectsEffectScale = $.effects.define("scale", function (options, done) {
    ...
    var el = $(this),
      mode = options.mode,
      percent =
        parseFloat(options.percent, 10) ||
        (parseFloat(options.percent, 10) === 0
          ? 0
          : mode !== "effect"
          ? 0
          : 100),
  ...
  var effectsEffectPuff = $.effects.define(
    ...
    function (options, done) {
      var newOptions = $.extend(true, {}, options, {
        fade: true,
        percent: parseFloat(options.percent, 10) || 150,
      });
  ...
  var widgetsAccordion = $.widget("ui.accordion", {
    ...
    _toggle: function (data) {
      ...
      if (toShow.length && toHide.length) {
        ...
        this.headers
          .filter(function () {
            return parseFloat($(this).attr("tabIndex"), 10) === 0;
          })
          .attr("tabIndex", -1);
      }
  ...
  $.extend(Datepicker.prototype, {
        ...
        shortYearCutoff =
          typeof shortYearCutoffTemp !== "string"
            ...
            : (new Date().getFullYear() % 100) +
              parseFloat(shortYearCutoffTemp, 10),
        dayNamesShort =
        ...
        offsetString = function (offset) {
          ...
          while (matches) {
            switch (matches[2] || "d") {
              case "d":
              case "D":
                day += parseFloat(matches[1], 10);
                break;
              case "w":
              case "W":
                day += parseFloat(matches[1], 10) * 7;
                break;
              case "m":
              case "M":
    ...
    _generateHTML: function (inst) {
      ...
      firstDay = parseFloat(this._get(inst, "firstDay"), 10);
        ...
        if (secondary || !changeYear) {
          ...
          determineYear = function (value) {
            var year = value.match(/c[+\-].*/)
              ? drawYear + parseFloat(value.substring(1), 10)
              : value.match(/[+\-].*/)
              ? thisYear + parseFloat(value, 10)
              : parseFloat(value, 10);
    ...
    _isInRange: function (inst, date) {
      ...
      if (years) {
        ...
        minYear = parseFloat(yearSplit[0], 10);
        maxYear = parseFloat(yearSplit[1], 10);
  ...
  $.widget("ui.draggable", $.ui.mouse, {
    ...
    _mouseStop: function (event) {
      ...
      if (
        ...
        $(this.helper).animate(
          ...
          parseFloat(this.options.revertDuration, 10),
          function () {
    ...
    _getParentOffset: function () {
      ...
      return {
        top:
          po.top +
          (parseFloat(this.offsetParent.css("borderTopWidth"), 10) || 0),
        left:
          po.left +
          (parseFloat(this.offsetParent.css("borderLeftWidth"), 10) || 0),
      };
    },
    ...
    _getRelativeOffset: function () {
      ...
      return {
        top:
          p.top -
          (parseFloat(this.helper.css("top"), 10) || 0) +
          (!scrollIsRootNode ? this.scrollParent.scrollTop() : 0),
        left:
          p.left -
          (parseFloat(this.helper.css("left"), 10) || 0) +
    ...
    _setContainment: function () {
      ...
      this.containment = [
        (parseFloat(c.css("borderLeftWidth"), 10) || 0) +
          (parseFloat(c.css("paddingLeft"), 10) || 0),
        (parseFloat(c.css("borderTopWidth"), 10) || 0) +
          (parseFloat(c.css("paddingTop"), 10) || 0),
        (isUserScrollable
          ? Math.max(ce.scrollWidth, ce.offsetWidth)
          : ce.offsetWidth) -
          (parseFloat(c.css("borderRightWidth"), 10) || 0) -
          (parseFloat(c.css("paddingRight"), 10) || 0) -
          this.helperProportions.width -
          this.margins.left -
          this.margins.right,
        (isUserScrollable
          ...
          : ce.offsetHeight) -
          (parseFloat(c.css("borderBottomWidth"), 10) || 0) -
          (parseFloat(c.css("paddingBottom"), 10) || 0) -
          this.helperProportions.height -
          this.margins.top -
          this.margins.bottom,
      ];
    ...
    start: function (event, ui, instance) {
      var min,
        ...
        group = $.makeArray($(o.stack)).sort(function (a, b) {
          return (
            (parseFloat($(a).css("zIndex"), 10) || 0) -
            (parseFloat($(b).css("zIndex"), 10) || 0)
          );
        });
      ...
      min = parseFloat($(group[0]).css("zIndex"), 10) || 0;
  ...
  var widgetsSortable = $.widget("ui.sortable", $.ui.mouse, {
    ...
    _mouseStop: function (event, noPropagation) {
      ...
      if (this.options.revert) {
        ...
        $(this.helper).animate(
          animation,
          parseFloat(this.options.revert, 10) || 500,
          function () {
            that._clear(event);
          }
        );
            ...
            if (!p.height()) {
              p.height(
                that.currentItem.innerHeight() -
                  parseFloat(that.currentItem.css("paddingTop") || 0, 10) -
                  parseFloat(that.currentItem.css("paddingBottom") || 0, 10)
              );
            }
            if (!p.width()) {
              p.width(
                that.currentItem.innerWidth() -
                  parseFloat(that.currentItem.css("paddingLeft") || 0, 10) -
                  parseFloat(that.currentItem.css("paddingRight") || 0, 10)
              );
            }
          },
        };
    ...
    _setContainment: function () {
      ...
      if (!/^(document|window|parent)$/.test(o.containment)) {
        ...
        this.containment = [
          co.left +
            (parseFloat($(ce).css("borderLeftWidth"), 10) || 0) +
            (parseFloat($(ce).css("paddingLeft"), 10) || 0) -
            this.margins.left,
          co.top +
            (parseFloat($(ce).css("borderTopWidth"), 10) || 0) +
            (parseFloat($(ce).css("paddingTop"), 10) || 0) -
            this.margins.top,
          co.left +
            (over ? Math.max(ce.scrollWidth, ce.offsetWidth) : ce.offsetWidth) -
            (parseFloat($(ce).css("borderLeftWidth"), 10) || 0) -
            (parseFloat($(ce).css("paddingRight"), 10) || 0) -
            this.helperProportions.width -
            this.margins.left,
          co.top +
            (over
              ...
              : ce.offsetHeight) -
            (parseFloat($(ce).css("borderTopWidth"), 10) || 0) -
            (parseFloat($(ce).css("paddingBottom"), 10) || 0) -
            this.helperProportions.height -
            this.margins.top,
        ];
      }
