(function () {
  ...
  require.helper.semVerSort = function (a, b) {
    ...
    for (var i = 0; i < aArray.length; ++i) {
      var aInt = parseFloat(aArray[i], 10);
      var bInt = parseFloat(bArray[i], 10);
      if (aInt === bInt) {
    ...
    Switchery.prototype.setPosition = function (clicked) {
      ...
      if (checked === true) {
        ...
        if (window.getComputedStyle)
          jack.style.left =
            parseFloat(window.getComputedStyle(switcher).width) -
            parseFloat(window.getComputedStyle(jack).width) +
            "px";
        else
          jack.style.left =
            parseFloat(switcher.currentStyle["width"]) -
            parseFloat(jack.currentStyle["width"]) +
            "px";
