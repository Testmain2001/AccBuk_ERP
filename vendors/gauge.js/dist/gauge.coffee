	...
	configPercentColors: () ->
		...
		if (@options.percentColors != undefined)
			...
			for i in [0..(@options.percentColors.length-1)]
				rval = parseFloat((cutHex(@options.percentColors[i][1])).substring(0,2),16)
				gval = parseFloat((cutHex(@options.percentColors[i][1])).substring(2,4),16)
				bval = parseFloat((cutHex(@options.percentColors[i][1])).substring(4,6),16)
