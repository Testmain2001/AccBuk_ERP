  ...
  var init = function () {
    ...
    for (var i = 0, inp; (inp = inputs[i]); i++) {
      if (
        ...
        options.step =
          getAttribute(inp, "step").search(/^any$/i) != -1
            ? options.max - options.min
            : getAttribute(inp, "step").search(stepRegExp) != -1
  ...
  function fdRange(options) {
    ...
    function onMouseDown(e) {
      ...
      if (targ.className.search("fd-slider-handle") != -1) {
        ...
        handlePos =
          parseFloat(vertical ? handle.offsetTop : handle.offsetLeft) || 0;
    ...
    function onTimer() {
      var xtmp = parseFloat(
        vertical ? handle.offsetTop : handle.offsetLeft,
        10
      );
      xtmp = Math.round(
        destPos < xtmp
    ...
    function tweenTo(tx) {
      kbEnabled = false;
      tweenX = parseFloat(tx, 10);
      tweenB = parseFloat(vertical ? handle.offsetTop : handle.offsetLeft, 10);
