...
<div class="row">     
	<div class="col-md-3">       
	<h4 class="fw-bold mb-4" style="padding-top:2px;">Cash Receipt Registor </h4>
	</div>
  ...
  <div class="card-datatable table-responsive pt-0 " style="overflow-x: auto;">
	...
	<tbody>
	   <?php
			...
			if($_REQUEST['Task']=='filter'&& $_REQUEST['customer']!=""|| $_REQUEST['location']!="" ){
				$cnd="receiptdate>='".$_SESSION['FromDate']."'AND receiptdate<='".$_SESSION['ToDate']."'AND customer like '%".$_REQUEST['customer']."%'  AND location like '%".$_REQUEST['location']."%'";
			}else{
				$cnd="receiptdate>='".$_SESSION['FromDate']."' AND receiptdate<='".$_SESSION['ToDate']."'";
			}
			 
			
			$data=$utilObj->getMultipleRow("sale_receipt"," $cnd AND payment_method='cash'");
			foreach($data as $info){
				$i++;
...
function Search(){
	...
	window.location="cash_receipt_registor_list.php?FromDate="+fromdate+"&ToDate="+todate+"&customer="+customer+"&location="+location+"&Task=filter";
